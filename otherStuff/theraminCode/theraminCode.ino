...
// C Major notes
int cMajor[] = { NOTE_C3, NOTE_D3, NOTE_E3, NOTE_G3, NOTE_A3 }
// G Major notes
int gMajor[] = { NOTE_G3, NOTE_A3, NOTE_B3, NOTE_D3, NOTE_E3}
// A Major notes
int aMajor[] = { NOTE_A3, NOTE_C3, NOTE_D3, NOTE_E3, NOTE_G3 }
// E Minor notes
int eMinor[] = { NOTE_E3, NOTE_G3, NOTE_A3, NOTE_B3, NOTE_D3 }

// Sensor Values
int sensorMin = 100; // REPLACE WHEN TESTED
int sensorMax = 800; // REPLACE WHEN TESTED
...
// Function: Map to the range of notes (in one of the above scales)
int maptoScale(int noteValue, int noteArray[]) { // input is scale and value of note
  int tempNote = map(value, sensorMin, sensorMax, 1, 6); // map to one of 5 vals 
  return noteArray[tempNote]; // set to clamped range
}
...
void loop() {
  ...
  note = maptoScale(value, cMajor); // map to scale and return corresponding note
